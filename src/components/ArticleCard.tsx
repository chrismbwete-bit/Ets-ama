...
interface ArticleCardProps {
  article: Article;
  onOrder: () => void; // juste déclencher la commande
  onView: () => void; // voir détails
}

export function ArticleCard({ article, onOrder, onView }: ArticleCardProps) {
  return (
    <div className="card-dark rounded-2xl overflow-hidden group">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex gap-2 pt-1">
          <button
            onClick={onView}
            className="flex-1 flex items-center justify-center gap-1 py-2.5 px-3 border border-cyan-500/30 text-cyan-400 rounded-xl hover:bg-cyan-500/10 transition-all text-sm font-medium"
          >
